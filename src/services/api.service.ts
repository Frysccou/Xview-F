...
export const ApiService = {
    login: async (data: LoginData): Promise<LoginResponse> => {
        const response = await HttpService.post<LoginResponse>(`${API_URL}/users/login`, data);
        if (response.token) {
    ...
    getProducts: async (): Promise<IProduct[]> => {
        const response = await HttpService.get<IProduct[]>(`${API_URL}/products`);
    ...
    createOrder: async (data: CreateOrderData): Promise<IOrder> => {
        ...
        const orderData = {
            ...
            productQuantities: data.products.reduce((acc, product) => {
    ...
    getUserOrders: async (): Promise<IOrder[]> => {
        ...
        const orders = await HttpService.get<IOrder[]>(`${API_URL}/users/orders`);
